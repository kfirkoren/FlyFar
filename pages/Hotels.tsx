...
import { useHotelsQuery } from '../services/queries';
import Reveal from '../components/Reveal';

const Hotels: React.FC = () => {
  const { data, isLoading, error } = useHotelsQuery();
  const hotels = data && data.length ? data : HOTELS;
  const errorMessage = error ? 'לא הצלחנו לטעון מלונות, מציג ברירת מחדל.' : '';
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {hotels.map((hotel, idx) => (
                <Reveal key={hotel.id} delay={idx * 120}>
